...
client.on("messageCreate", (message) => {
    ...
    switch (content) {
        case "명령어":
        case "help":
            const embed = new discord_js_1.EmbedBuilder({ title: "명령어" })
                .addFields({
    ...
    if (content.startsWith("cr ")) {
        ...
        if (datas.length != 5) {
            message.channel.send("잘못 입력하셨습니다. 사용법: !cr 근접전수치 회피/반격수치 피해(nDm) 피해보너스(nDm/수치) 반격데미지(nDm)");
        }
        else {
            const embed = new discord_js_1.EmbedBuilder({
            ...
            if (cr > cr_percent) {
                ...
                const results = [];
                if (cr > Math.floor(cr_percent / 2))
                    results.push(1);
                else if (cr > Math.floor(cr_percent / 5))
                    results.push(2);
                else
                    results.push(3);
                if (avoid > avoid_percent)
                    results.push(0);
                else if (avoid > Math.floor(avoid_percent / 2))
                    results.push(1);
                else if (avoid > Math.floor(avoid_percent / 5))
                    results.push(2);
                else
                    results.push(3);
                if (results[0] < results[1]) {
                    ...
                    for (let i = 0; i < parseInt(counter_attack[0]); i++) {
                        const current_damage = GenerateRandomNumber(parseInt(counter_attack[1]));
                    ...
                    if ((damage_bonus.includes("d") && damage_bonus.length != 2) ||
                        damage.length != 2) {
                        message.channel.send("데미지를 잘못 입력하셨습니다.");
                        return;
                    }
                    let damage_sum = 0;
                    let damage_msg = `데미지(${parseInt(damage[0])}D${parseInt(damage[1])}): `;
                    ...
                    if (results[0] == 3)
                        damage_msg += "(극단적 성공 -> 최대 데미지)";
                    if (damage_bonus.includes("d")) {
                        damage_msg += `\n데미지 보너스(${parseInt(damage_bonus[0])}D${parseInt(damage_bonus[1])}): `;
                        for (let i = 0; i < parseInt(damage_bonus[0]); i++) {
                            ...
                            if (results[0] == 3) {
                                ...
                                current_damage = GenerateRandomNumber(parseInt(damage_bonus[1]));
                            }
                            damage_sum += current_damage;
                            damage_msg += `${current_damage} `;
                        }
...
client.once("ready", () => {
    var _a, _b;
    (_a = client.user) === null || _a === void 0 ? void 0 : _a.setPresence({ activities: [{ name: "!명령어" }] });
